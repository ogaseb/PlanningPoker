...
class CardResults extends Component {
  ...
  render () {
    const {
      store: {
        roomStore: { waiting, cardResults, setSelectedCard, selectedCard }
      }
    } = this.props
    return (
      <React.Fragment>
        <Wrapper>
          <CardsWrapperWaiting>
            ...
            {selectedCard && (
              <CardMargin>
                <StyledCard color={'#303F9F'}>{selectedCard}</StyledCard>
              </CardMargin>
            )}
          </CardsWrapperWaiting>
          <CardsWrapperResults>
              ...
              cardResults.map((result, index) => {
                return (
                  <ReactCardFlip
                    ...
                    isFlipped={this.isFlipped[index]}
                  >
