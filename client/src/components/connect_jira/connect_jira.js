...
class ConnectJira extends Component {
  ...
  componentDidMount() {
    if (localStorage.getItem("jira-credentials") !== null && localStorage.getItem("jira-subdomains") !== null) {
      ...
      this.props.store.jiraStore.jiraLogin(subdomains[0], data.jiraLogin, data.jiraPassword)
    }
    window.onpopstate = this.onBackButtonEvent
  }

  onBackButtonEvent = (e) => {
    const {store: {userStore: {leaveRoom}}, history: {push}} = this.props
    leaveRoom()
    push(routes.root())
  };

  render() {
    const {store: {jiraStore: {jiraLoggedIn}}} = this.props
    return (
